...
다양한 시도 끝에 selenium 라이브러리를 이용하여 추가적인 데이터를 수집하려 했으나 selenium 라이브러리를 이용하더라도 네이버 쇼핑몰의 스크립트에는 접근이 힘들었습니다.  
그러하여 webbrowser 라이브러리를 통해 해당 페이지에 접속하고 pyautogui 라이브러리의 이미지 검색과 pyperclip 라이브러리의 클립보드 기능을 같이 이용하여 데이터를 수집하였습니다.
